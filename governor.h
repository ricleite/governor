...
// macro user API
#define GOV_RESET() sGovernor->Reset()
#define GOV_PREPARE(numThreads) sGovernor->Prepare(numThreads)
#define GOV_SUBSCRIBE(threadId) sGovernor->Subscribe(threadId)
#define GOV_CONTROL() sGovernor->ControlPoint()
...
private:
    ...
    // if atExit = true, closes all handles
